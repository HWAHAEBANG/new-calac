...
const FindPwBox = () => {
  ...
  const handleFindPw = () => {
    axios
      .get(
        `https://calac.herokuapp.com/login/findPw?inputId=${identification.id}&inputQuiz=${identification.quiz}&inputAnswer=${identification.answer}`
      )
      .then((response) => {
        if (response.data) {
          setIdentified({
  ...
  const handleChangePw = () => {
    axios // 새 이벤트 DB에 UPDATE
      .put(`https://calac.herokuapp.com/login/findPw/changePw`, {
